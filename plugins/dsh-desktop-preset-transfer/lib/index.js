// Ported from dataelement/dsh-desktop (MIT): packages/dsh-desktop-preset-transfer.
// Registered on the official `connection.fetch` route registry (dsh-client-connection),
// the same seam upstream's own /api/session.export uses.
import { chmod, lstat, mkdir, mkdtemp, readFile, readdir, rename, rm, stat, writeFile } from 'node:fs/promises'
import { dirname, extname, join, resolve, sep } from 'node:path'
import { strFromU8, strToU8, unzipSync, zipSync } from 'fflate'
import { COMPOSITION_FILE, scanRoot, writableRoot } from '@deepseek-ai/dsh-agent-presets'
