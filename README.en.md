...
## 🗒️ This release (v1.4.x — standalone 1.x line)

- **Version line switch**: since **v1.4.0** the desktop version is decoupled from upstream `@deepseek-ai/dsh` and becomes its own incrementing patch line (`1.4.0 → 1.4.1 → …`); the upstream version is tracked in `dsh.upstreamVersion` instead. This keeps electron-updater's requirement of valid, strictly increasing semver while dropping the old "upstream version + UTC timestamp" scheme, which would keep minting meaningless timestamps once the desktop line passed the upstream version.
- **First 1.x installer**: the first package on this line (v1.4.0+) carries all the fixes and features accumulated since v0.1.2:
  - **Auto-update 404 fixed**: `publish.owner` mismatched the actual publishing repo, so electron-updater polled a nonexistent repo every 4 hours; corrected to `lxfebd/dsh-desktop-unified`.
  - **"Pull updates from GitHub…"** (tray + app menu): GitHub API latest-release lookup → semver compare (handles `-rc.x` prereleases) → platform installer pick → streaming download (progress notification) → open/reveal installer; network failures offer the releases page.
  - **8 defects fixed**: dshmarket's running-agent guard self-blocking AI-driven plugin installs; session-completion notifications firing for stale pre-start sessions; boot preheat deleting real package dirs under the profile; the shortcut-icon updater rewritten from an invalid PowerShell script (parse error) to a correct target-filtered implementation; `preset-deps` misclassifying `~0.1.2` tilde semver ranges as local paths (breaking every market install); the shell-control restart timer not being cleared (possible second relaunch after quit); etc.
- **Quality gates**: Vitest 10 files 68 cases + plugin node:test 5 cases all green; `tsc --noEmit` 0 errors.
...
6. **Don't hand-edit the version number.** The desktop version is a standalone incrementing 1.x line (since v1.4.0, computed by `nextVersion()` in `sync-upstream.mjs`: 1.4.0 → 1.4.1 → …); the upstream `@deepseek-ai/dsh` version lives in `dsh.upstreamVersion` and is no longer encoded in the desktop number. The old scheme (upstream prerelease + UTC timestamp, e.g. `0.1.2-rc.1.202609041312`) is legacy; if the version ever regresses to that shape, `nextVersion()` folds it back to the stable core (`0.1.2` → `0.1.3`) and rejoins the 1.x line. `semver.inc` guarantees valid, strictly increasing semver (electron-updater requirement).
...
- **Unit tests**: Vitest 10 files 68 cases (incl. `sync-upstream`, `market-tools.*`, `github-releases`, `session-notifier`), all passing
