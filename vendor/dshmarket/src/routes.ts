...
export function mountMarketRoutes(
  ...
  /**
   * Running-agent ids for the mutation gate, excluding the request's
   * originating agent. The originator is mid-turn by definition while it
   * performs the mutation, so its own running state must not block it (that
   * would make every agent-initiated install/update/uninstall self-block);
   * other running agents still do. Reads the caller id from the request body
   * (`agentId`), which dsh-market-tools sends on agent-initiated mutations.
   */
  const runningAgentsForGuard = (request?: { body?: { agentId?: unknown } }): string[] => {
    const service = agentsLookup?.()
    const requestedId = request?.body?.agentId
    const excludeId = typeof requestedId === 'string' && requestedId !== '' ? requestedId : undefined
    const ids = runningAgentIds(service, excludeId)
  ...
  const disposers = [
    ...
    host.webServer.register(captureLegacy('/dsh-market/update', {
      ...
      handler: async (request, response) => {
        ...
        try {
          await withMutationLock(response, 'install', async () => {
            const body = (await readJsonBody(request)) as { name?: unknown; force?: unknown; restore?: unknown; agentId?: unknown }
            const name = typeof body.name === 'string' ? body.name : ''
            ...
            // (The agent that ISSUED this request is excluded: it is running
            // by definition while it calls the market, and blocking on its own
            // state would make every agent-initiated update self-block.)
            const busyAgents = runningAgentsForGuard({ body })
    ...
    host.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          await withMutationLock(response, 'install', async () => {
            const body = (await readJsonBody(request)) as { name?: unknown; force?: unknown; agentId?: unknown }
            const name = typeof body.name === 'string' ? body.name : ''
    ...
    host.webServer.register({
      ...
      handler: async (request, response) => {
        ...
        try {
          await withMutationLock(response, 'install', async () => {
            const body = (await readJsonBody(request)) as { url?: unknown; agentId?: unknown }
            const busyAgents = runningAgentsForGuard({ body })
            if (busyAgents.length > 0) {
