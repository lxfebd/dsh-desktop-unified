 ...
 *
 * `excludeId` lets a mutating request (install/update/uninstall) name the
 * agent that originated it: that agent is mid-turn by definition while it
 * issues the request, so blocking on its own running state would make every
 * agent-initiated mutation self-block. Only OTHER running agents still
 * block, which preserves the mixed-files protection for everyone else.
 */
export function runningAgentIds(agents: AgentsServiceLike | undefined, excludeId?: string): string[] {
